...
int PythonQtInstanceWrapper_init(PythonQtInstanceWrapper* self, PyObject* args, PyObject* kwds) {
  ...
  if (self->classInfo()->constructors()) {
    ...
    PythonQtSlotFunction_CallImpl(self->classInfo(), nullptr, self->classInfo()->constructors(), args, kwds, nullptr, &directCPPPointer,
                                  &ownership);
    if (PyErr_Occurred()) {
      return -1;
    }
...
static PyObject* PythonQtInstanceWrapper_richcompare(PythonQtInstanceWrapper* wrapper, PyObject* other, int code) {
  ...
  if (opSlot._type == PythonQtMemberInfo::Slot) {
    ...
    PyObject* result =
        PythonQtSlotFunction_CallImpl(wrapper->classInfo(), wrapper->_obj, opSlot._slot, args, nullptr, wrapper->_wrappedPtr);
    Py_DECREF(args);
    if (result == nullptr) {
...
PyObject* PythonQtInstanceWrapper_delete(PythonQtInstanceWrapper* self, PyObject*) {
  ...
  if (deleteSlot._type == PythonQtMemberInfo::Slot) {
    ...
    PyObject* resultObj =
        PythonQtSlotFunction_CallImpl(self->classInfo(), self->_obj, deleteSlot._slot, nullptr, nullptr, self->_wrappedPtr);
...
static PyMethodDef PythonQtInstanceWrapper_methods[] = {
    ...
    {"inherits", (PyCFunction)PythonQtInstanceWrapper_inherits, METH_VARARGS, "Returns if the class inherits or is of given type name"},
    {"help", (PyCFunction)PythonQtInstanceWrapper_help, METH_NOARGS, "Shows the help of available methods for this class"},
    {"delete", (PyCFunction)PythonQtInstanceWrapper_delete, METH_NOARGS, "Deletes the C++ object (at your own risk, my friend!)"},
...
static PyObject* PythonQtInstanceWrapper_getattro(PyObject* obj, PyObject* name) {
  ...
  if (qstrcmp(attributeName, "__dict__") == 0) {
    ...
    if (wrapper->_obj) {
      ...
      Q_FOREACH (QString name, l) {
        ...
        if (o) {
          ...
          std::cerr << "PythonQtInstanceWrapper: something is wrong, could not get attribute " << QStringToPythonConstCharPointer(name)
                    << std::endl;
        }
      }
...

    {
      ...
      if (member._type == PythonQtMemberInfo::Slot) {
        ...
        PyObject* result =
            PythonQtSlotFunction_CallImpl(wrapper->classInfo(), wrapper->_obj, member._slot, args, nullptr, wrapper->_wrappedPtr);
        Py_DECREF(args);
        if (result) {
  ...
  case PythonQtMemberInfo::Property:
    if (wrapper->_obj) {
      ...
      QString error =
          QString("Trying to read property '") + attributeName + "' from a destroyed " + wrapper->classInfo()->className() + " object";
      PyErr_SetString(PyExc_ValueError, QStringToPythonConstCharPointer(error));
      return nullptr;
    }
    break;
  ...
  if (member._type == PythonQtMemberInfo::Property) {
    ...
    if (prop.isWritable()) {
      ...
      if (success) {
        ...
        error = QString("Property '") + attributeName + "' of type '" + prop.typeName() + "' does not accept an object of type " +
...
static QString getStringFromObject(PythonQtInstanceWrapper* wrapper) {
  ...
  if (wrapper->_wrappedPtr || wrapper->_obj) {
    ...
    if (info._type == PythonQtMemberInfo::Slot) {
      PyObject* resultObj =
          PythonQtSlotFunction_CallImpl(wrapper->classInfo(), wrapper->_obj, info._slot, nullptr, nullptr, wrapper->_wrappedPtr);
      if (resultObj) {
...
static PyObject* PythonQtInstanceWrapper_str(PyObject* obj) {
  ...
  if (wrapper->_wrappedPtr) {
    if (wrapper->_obj) {
      return PyString_FromFormat("%s (C++ Object %p wrapped by %s %p))", typeName, wrapper->_wrappedPtr,
...
static PyObject* PythonQtInstanceWrapper_repr(PyObject* obj) {
  ...
  if (!str.isEmpty()) {
    if (str.startsWith(typeName)) {
      ...
      return PyString_FromFormat("%s (%s, at: %p)", typeName, QStringToPythonConstCharPointer(str),
                                 wrapper->_wrappedPtr ? wrapper->_wrappedPtr : qobj);
    }
  }
  if (wrapper->_wrappedPtr) {
    if (wrapper->_obj) {
      return PyString_FromFormat("%s (C++ object at: %p wrapped by %s at: %p)", typeName, wrapper->_wrappedPtr,
                                 wrapper->_obj->metaObject()->className(), qobj);
    } else {
...
PyTypeObject PythonQtInstanceWrapper_Type = {
    PyVarObject_HEAD_INIT(&PythonQtClassWrapper_Type, 0) "PythonQt.PythonQtInstanceWrapper", /*tp_name*/
