...
PythonQtSlotInfo* PythonQtClassInfo::recursiveFindDecoratorSlotsFromDecoratorProvider(const char* memberName, PythonQtSlotInfo* inputInfo,
                                                                                      bool& found,
                                                                                      QHash<QByteArray, PythonQtMemberInfo>& memberCache,
                                                                                      int upcastingOffset) {
  ...
  Q_FOREACH (const ParentClassInfo& info, _parentClasses) {
    inputInfo = info._parent->recursiveFindDecoratorSlotsFromDecoratorProvider(memberName, inputInfo, found, memberCache,
                                                                               upcastingOffset + info._upcastingOffset);
  }
  return inputInfo;
}

PythonQtSlotInfo* PythonQtClassInfo::findDecoratorSlotsFromDecoratorProvider(const char* memberName, PythonQtSlotInfo* tail, bool& found,
                                                                             QHash<QByteArray, PythonQtMemberInfo>& memberCache,
                                                                             int upcastingOffset) {
    ...
    for (int i = startFrom; i < numMethods; i++) {
      ...
      if ((m.methodType() == QMetaMethod::Method || m.methodType() == QMetaMethod::Slot) && m.access() == QMetaMethod::Public) {
        ...
        if (signature == memberName) {
          found = true;
          PythonQtSlotInfo* info = new PythonQtSlotInfo(
              this, m, i, decoratorProvider, isClassDeco ? PythonQtSlotInfo::ClassDecorator : PythonQtSlotInfo::InstanceDecorator);
...
PythonQtMemberInfo PythonQtClassInfo::member(const char* memberName) {
  ...
  if (info._type != PythonQtMemberInfo::Invalid) {
    ...
    if (!found) {
      ...
      if ((mbrName.length() > 2) && (mbrName.at(mbrName.length() - 1) == '_') && (mbrName.at(mbrName.length() - 2) != '_')) {
...
const QByteArray& PythonQtClassInfo::className() const { return _wrappedClassName; }
...
void PythonQtClassInfo::addDecoratorSlot(PythonQtSlotInfo* info) { _decoratorSlots.append(info); }
...
void PythonQtClassInfo::addNestedClass(PythonQtClassInfo* info) { _nestedClasses.append(info); }
