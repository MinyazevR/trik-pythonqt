...
void PythonQtPrivate::setTaskDoneCallback(const PythonQtObjectPtr& callable) { _pyTaskDoneCallback = callable; }
...
void PythonQt::setQObjectMissingAttributeCallback(PythonQtQObjectMissingAttributeCB* cb) { _self->_p->_qObjectMissingAttribCB = cb; }
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (!keystr.startsWith(underscoreStr)) {
          ...
          case Class:
            ...
            break;
          case Variable:
            if (value->ob_type != &PyCFunction_Type && value->ob_type != &PyFunction_Type && value->ob_type != &PyMethod_Type &&
                value->ob_type != &PyModule_Type && value->ob_type != &PyType_Type && value->ob_type != &PythonQtSlotFunction_Type
#ifndef PY3K
...
void PythonQt::addInstanceDecorators(QObject* o) { _p->addDecorators(o, PythonQtPrivate::InstanceDecorator); }
...
void PythonQt::addDecorators(QObject* o) { _p->addDecorators(o, PythonQtPrivate::AllDecorators); }

void PythonQt::registerQObjectClassNames(const QStringList& names) { _p->registerQObjectClassNames(names); }
...
void PythonQt::setImporterIgnorePaths(const QStringList& paths) { _p->_importIgnorePaths = paths; }

const QStringList& PythonQt::getImporterIgnorePaths() { return _p->_importIgnorePaths; }

void PythonQt::addWrapperFactory(PythonQtCppWrapperFactory* factory) { _p->_cppWrapperFactories.append(factory); }

void PythonQt::addWrapperFactory(PythonQtForeignWrapperFactory* factory) { _p->_foreignWrapperFactories.append(factory); }
...
void PythonQtPrivate::addDecorators(QObject* o, int decoTypes) {
  ...
  for (int i = 0; i < numMethods; i++) {
    ...
    if ((m.methodType() == QMetaMethod::Method || m.methodType() == QMetaMethod::Slot) && m.access() == QMetaMethod::Public) {
...
void PythonQtPrivate::removeSignalEmitter(QObject* obj) { _signalReceivers.remove(obj); }
...
void PythonQt::setSystemExitExceptionHandlerEnabled(bool value) { _p->_systemExitExceptionHandlerEnabled = value; }

bool PythonQt::systemExitExceptionHandlerEnabled() const { return _p->_systemExitExceptionHandlerEnabled; }
...
void PythonQt::setQObjectWrappedCallback(PythonQtQObjectWrappedCB* cb) { _p->_wrappedCB = cb; }

void PythonQt::setQObjectNoLongerWrappedCallback(PythonQtQObjectNoLongerWrappedCB* cb) { _p->_noLongerWrappedCB = cb; }
...
#ifdef PY3K
static PyModuleDef PythonQtModuleDef = {PyModuleDef_HEAD_INIT, "", nullptr, -1, PythonQtMethods, nullptr, nullptr, nullptr, nullptr};
#endif
...
void PythonQt::addPolymorphicHandler(const char* typeName, PythonQtPolymorphicHandlerCB* cb) { _p->addPolymorphicHandler(typeName, cb); }
...
void PythonQtPrivate::handleVirtualOverloadReturnError(const char* signature, const PythonQtMethodInfo* methodInfo, PyObject* result) {
  QString error = "Return value '" + PythonQtConv::PyObjGetString(result) + "' can not be converted to expected C++ type '" +
                  methodInfo->parameters().at(0).name + "' as return value of virtual method " + signature;
...
void PythonQt::removeWrapperFactory(PythonQtCppWrapperFactory* factory) { _p->_cppWrapperFactories.removeAll(factory); }

void PythonQt::removeWrapperFactory(PythonQtForeignWrapperFactory* factory) { _p->_foreignWrapperFactories.removeAll(factory); }

void PythonQtPrivate::removeWrapperPointer(void* obj) { _wrappedObjects.remove(obj); }

void PythonQtPrivate::addWrapperPointer(void* obj, PythonQtInstanceWrapper* wrapper) { _wrappedObjects.insert(obj, wrapper); }
...
PythonQtClassInfo* PythonQtPrivate::getClassInfo(const QMetaObject* meta) { return getClassInfo(QByteArray(meta->className())); }
