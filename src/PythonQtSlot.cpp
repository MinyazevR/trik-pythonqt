...
bool PythonQtCallSlot(PythonQtClassInfo* classInfo, QObject* objectToCall, PyObject* args, bool strict, PythonQtSlotInfo* info,
                      void* firstArgument, PyObject** pythonReturnValue, void** directReturnValuePointer,
  ...
  for (int i = 1 + instanceDecoOffset; i < argc && ok; i++) {
    ...
    argList[i] =
        PythonQtConv::ConvertPythonToQt(param, PyTuple_GET_ITEM(args, i - 1 - instanceDecoOffset), strict, classInfo, nullptr, frame);
    ...
    if (!hadException) {
      if (argList[0] || isVoidReturnValue) {
        ...
        QString e = QString("Called ") + info->fullSignature() + ", return type '" + returnValueParam.name +
                    "' is ignored because it is unknown to PythonQt. Probably you should register it using qRegisterMetaType() or add a "
                    "default constructor decorator to the class.";
...
PyObject* PythonQtMemberFunction_Call(PythonQtSlotInfo* info, PyObject* m_self, PyObject* args, PyObject* kw) {
  if (PyObject_TypeCheck(m_self, &PythonQtInstanceWrapper_Type)) {
    ...
    if (!info->isClassDecorator() && (self->_obj == nullptr && self->_wrappedPtr == nullptr)) {
      ...
      PyObject* result =
          PythonQtSlotFunction_CallImpl(self->classInfo(), self->_obj, info, args, kw, self->_wrappedPtr, nullptr, &ownership);
    ...
    if (info->isClassDecorator()) {
      ...
      if (argc > 0) {
        ...
        if (PyObject_TypeCheck(firstArg, (PyTypeObject*)&PythonQtInstanceWrapper_Type) &&
          ...
          if (!info->isClassDecorator() && (self->_obj == nullptr && self->_wrappedPtr == nullptr)) {
            QString error =
                QString("Trying to call '") + info->slotName() + "' on a destroyed " + self->classInfo()->className() + " object";
            PyErr_SetString(PyExc_ValueError, QStringToPythonConstCharPointer(error));
            return nullptr;
          }
          ...
          PythonQtPassThisOwnershipType ownership;
          PyObject* result =
              PythonQtSlotFunction_CallImpl(self->classInfo(), self->_obj, info, newargs, kw, self->_wrappedPtr, nullptr, &ownership);
          if (ownership == PassOwnershipToCPP) {
            self->passOwnershipToCPP();
          } else if (ownership == PassOwnershipToPython) {
            self->passOwnershipToPython();
...
PyObject* PythonQtSlotFunction_CallImpl(PythonQtClassInfo* classInfo, QObject* objectToCall, PythonQtSlotInfo* info, PyObject* args,
                                        PyObject* kw, void* firstArg, void** directReturnValuePointer,
                                        PythonQtPassThisOwnershipType* passThisOwnershipToCPP) {
  ...
  if ((kw != nullptr && PyDict_Check(kw) && (PyDict_Size(kw) > 0))) {
    ...
    if (kwSlotFound) {
      ...
      ok = PythonQtCallSlot(classInfo, objectToCall, combinedArgs, false, slotInfo, firstArg, &r, directReturnValuePointer,
                            passThisOwnershipToCPP);
      if (!ok && !PyErr_Occurred()) {
        QString e =
            QString("Called ") + info->fullSignature() + " with wrong arguments: " + limitString(PythonQtConv::PyObjGetString(args));
        PyErr_SetString(PyExc_ValueError, QStringToPythonConstCharPointer(e));
      }
    } else {
...
static PyObject* meth_get__doc__(PythonQtSlotFunctionObject* m, void* /*closure*/) {
  ...
  if (returnType == "QString" || returnType == "SbName" || returnType == "SbString") {
    ...
    pyReturnType = "object";
             ...
             returnTypeId == QMetaType::Long || returnTypeId == QMetaType::ULong || returnTypeId == QMetaType::LongLong ||
             returnTypeId == QMetaType::ULongLong) {
    pyReturnType = "int";
...
static PyObject* meth_get__name__(PythonQtSlotFunctionObject* m, void* /*closure*/) { return PyString_FromString(m->m_ml->slotName(true)); }
...
static PyMemberDef meth_members[] = {{const_cast<char*>("__module__"), T_OBJECT, OFF(m_module), PY_WRITE_RESTRICTED, 0},
                                     {nullptr, 0, 0, 0, 0}};
...
static PyObject* meth_repr(PythonQtSlotFunctionObject* f) {
  if (f->m_self->ob_type == &PythonQtClassWrapper_Type) {
    ...
    return PyString_FromFormat("<unbound qt slot %s of %s type>", f->m_ml->slotName().constData(),
                               self->classInfo()->className().constData());
  } else {
    return PyString_FromFormat("<qt slot %s of %s instance at %p>", f->m_ml->slotName().constData(), f->m_self->ob_type->tp_name,
                               f->m_self);
  }
}
...
PyTypeObject PythonQtSlotFunction_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "builtin_qt_slot",
